...
async function restart(service) {
    ...
    try {
        ...
        await loadServices();
    } catch (e) { error.value = e.response?.data?.message ?? 'Restart failed'; }
}
...
<template>
    ...
    <AuthenticatedLayout>
        <template #header>
            <h2 class="text-xl font-semibold leading-tight text-gray-800">IT-Ops · Services &amp; Logs</h2>
        </template>
        ...
        <div class="py-6">
            <div class="mx-auto max-w-[100rem] space-y-6 px-4 sm:px-6 lg:px-8">
                ...
                <!-- Services -->
                <section>
                    <h3 class="mb-2 text-sm font-semibold uppercase text-gray-500">Workers &amp; services</h3>
                    <div class="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4">
                        <div v-for="s in services" :key="s.service" class="rounded-lg bg-white p-4 shadow-sm">
                            ...
                            <div class="mt-1 text-xs text-gray-400">seen {{ s.lastSeenAt?.substring(11,19) || '—' }}</div>
                            <button class="mt-2 w-full rounded bg-indigo-50 px-2 py-1 text-xs text-indigo-700" @click="restart(s.service)">Restart</button>
                        </div>
                        <div v-if="!services.length" class="col-span-full rounded-lg bg-white p-4 text-sm text-gray-400 shadow-sm">
                            No heartbeats yet. Start a worker: <code>php artisan sophix:workflow:work</code>
                        </div>
                    </div>
                    <div class="mt-3 flex flex-wrap gap-4 text-xs text-gray-600">
                        <span>Workflow tasks queued: <strong>{{ queues.workflowTasksCreated ?? 0 }}</strong></span>
                        <span>Incidents: <strong class="text-red-600">{{ queues.workflowIncidents ?? 0 }}</strong></span>
                        <span>Outbox unpublished: <strong>{{ queues.outboxUnpublished ?? 0 }}</strong></span>
                    </div>
                </section>
                ...
                <section>
                    <div class="mb-2 flex items-center gap-2">
                        <h3 class="text-sm font-semibold uppercase text-gray-500">Logs</h3>
                        ...
                        <input v-model="filters.q" placeholder="search message" class="w-64 rounded border-gray-300 text-sm" @keyup.enter="loadLogs" />
                        <button class="rounded border bg-white px-3 py-1 text-sm" @click="loadLogs">Search</button>
                    </div>
                    <div class="max-h-[40vh] overflow-auto rounded-lg bg-gray-900 p-3 font-mono text-xs text-gray-100">
                        ...
                        <div v-if="!logs.length" class="text-gray-500">No log entries match.</div>
                    </div>
                </section>
            </div>
        </div>
