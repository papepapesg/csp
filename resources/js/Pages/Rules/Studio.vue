...
async function save() {
    ...
    try {
        ...
        if (current.value.table_id) {
            ...
            notice.value = 'Decision table updated.';
        } else {
        ...
        await load();
    } catch (e) { error.value = e.response?.data?.message ?? 'Save failed (check JSON)'; }
}
...
async function runTest() {
    ...
    } catch (e) { error.value = e.response?.data?.message ?? 'Evaluate failed'; }
}
...
<template>
    <Head title="Rules Studio" />
    <AuthenticatedLayout>
        <template #header>
            <h2 class="text-xl font-semibold leading-tight text-gray-800">Rules Studio (FOUNDATION_DROOLS)</h2>
        </template>

        <div class="py-6">
            <div class="mx-auto max-w-[100rem] px-4 sm:px-6 lg:px-8">
                <p class="mb-3 text-sm text-gray-500">Decision tables are configuration. Edit the rules, deploy a new version, and the engine applies the new policy live — operators override per market with no code change.</p>
                ...
                <div class="grid grid-cols-12 gap-4">
                    ...
                    <div class="col-span-3 rounded-lg bg-white p-3 shadow-sm">
                        <div class="mb-2 flex items-center">
                            <h3 class="flex-1 text-xs font-semibold uppercase text-gray-500">Decision tables</h3>
                            <button class="rounded bg-gray-800 px-2 py-1 text-xs text-white" @click="create">+ New</button>
                        </div>
                        <ul class="space-y-1 text-sm">
                            <li v-for="t in tables" :key="t.table_id">
                                <button class="w-full rounded px-2 py-1 text-left hover:bg-gray-100" :class="current?.table_id === t.table_id ? 'bg-gray-100 font-medium' : ''" @click="open(t)">
                                    {{ t.rule_set }} <span class="text-xs text-gray-400">v{{ t.version }}{{ t.operator_code ? ' · '+t.operator_code : '' }}</span>
                                </button>
                            </li>
                        </ul>
                    </div>
                    ...
                    <div class="col-span-6 space-y-3">
                        <div v-if="current" class="rounded-lg bg-white p-4 shadow-sm">
                            <div class="grid grid-cols-2 gap-3">
                                <div><div class="label text-xs text-gray-500">Rule set</div><input v-model="current.rule_set" :disabled="!!current.table_id" class="w-full rounded border-gray-300 text-sm" /></div>
                                <div><div class="text-xs text-gray-500">Name</div><input v-model="current.name" class="w-full rounded border-gray-300 text-sm" /></div>
                                <div><div class="text-xs text-gray-500">Operator (blank = global)</div><input v-model="current.operator_code" placeholder="WIK / WTZ…" class="w-full rounded border-gray-300 text-sm" /></div>
                                <div><div class="text-xs text-gray-500">Hit policy</div><select v-model="current.hit_policy" class="w-full rounded border-gray-300 text-sm"><option>FIRST</option><option>COLLECT</option></select></div>
                            </div>

                            <h4 class="mt-4 text-xs font-semibold uppercase text-gray-500">Rules (when → then)</h4>
                            <div v-for="(rule, ri) in current.rules" :key="ri" class="mt-2 rounded border p-2">
                                <div class="mb-1 flex items-center text-xs text-gray-500"><span class="flex-1">WHEN (all)</span><button class="text-red-500" @click="removeRule(ri)">remove rule</button></div>
                                <div v-for="(c, ci) in rule.when" :key="ci" class="mb-1 flex gap-1">
                                    <input v-model="c.var" placeholder="variable" class="flex-1 rounded border-gray-300 text-xs" />
                                    <select v-model="c.op" class="rounded border-gray-300 text-xs"><option v-for="o in ops" :key="o">{{ o }}</option></select>
                                    <input v-model="c.value" placeholder="value" class="w-24 rounded border-gray-300 text-xs" />
                                    <button class="px-1 text-red-400" @click="removeCond(rule, ci)">×</button>
                                </div>
                                <button class="text-xs text-indigo-600" @click="addCond(rule)">+ condition</button>
                                <div class="mt-1 text-xs text-gray-500">THEN (output JSON)</div>
                                <input v-model="rule.thenJson" class="w-full rounded border-gray-300 font-mono text-xs" />
                            </div>
                            <button class="mt-2 text-sm text-indigo-600" @click="addRule">+ rule</button>

                            <div class="mt-3 text-xs text-gray-500">Default output (no rule matched)</div>
                            <input v-model="current.defaultJson" class="w-full rounded border-gray-300 font-mono text-xs" />

                            <button class="mt-3 w-full rounded bg-indigo-600 py-2 text-sm font-semibold text-white" @click="save">{{ current.table_id ? 'Save table' : 'Deploy new table' }}</button>
                        </div>
                        <p v-else class="text-sm text-gray-400">Select or create a decision table.</p>
                    </div>

                    <!-- test -->
                    <div class="col-span-3">
                        <div v-if="current" class="rounded-lg bg-white p-4 shadow-sm">
                            <h3 class="mb-2 text-xs font-semibold uppercase text-gray-500">Test: {{ current.rule_set }}</h3>
                            <div class="text-xs text-gray-500">Facts (JSON)</div>
                            <textarea v-model="test.facts" rows="5" class="w-full rounded border-gray-300 font-mono text-xs"></textarea>
                            <button class="mt-2 w-full rounded bg-gray-800 py-1.5 text-sm text-white" @click="runTest">Evaluate</button>
