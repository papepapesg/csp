...
<template>
    ...
    <AuthenticatedLayout>
        <template #header><h2 class="font-semibold text-xl text-gray-800">{{ t('Commercial Studio — campaigns & bundles') }}</h2></template>
            ...
            <div v-if="tab === 'campaigns'">
                ...
                <div v-if="showDesigner" class="bg-white rounded shadow p-4 mb-4 space-y-3">
                    <div class="grid grid-cols-4 gap-2">
                        ...
                        <input v-model="draft.name" :placeholder="t('Display name')" class="border rounded px-2 py-1 text-sm" />
                        <select v-model="draft.campaign_type" class="border rounded px-2 py-1 text-sm"><option v-for="ct in campaignTypes" :key="ct">{{ ct }}</option></select>
                        <input v-model.number="draft.max_participants" type="number" :placeholder="t('Max participants')" class="border rounded px-2 py-1 text-sm" />
                    </div>
                    <div class="grid grid-cols-4 gap-2">
                        <label class="text-xs text-gray-500 self-center">{{ t('Window') }}</label>
                    ...
                    <div>
                        <div class="text-xs font-semibold text-gray-500 mb-1">{{ t('Offer (PLM-CFG-04 discount → SIP-03 assignment on redemption)') }}</div>
                    ...
                    <div>
                        <div class="text-xs font-semibold text-gray-500 mb-1">{{ t('Targeting rules') }} <button @click="addRule" class="px-1 bg-gray-200 rounded">+</button></div>
                        <div v-for="(r, i) in draft.target_rules" :key="i" class="flex gap-2 mb-1">
                            <select v-model="r.rule_type" class="border rounded px-2 py-1 text-sm"><option v-for="rt in ruleTypes" :key="rt">{{ rt }}</option></select>
                            ...
                            <input v-model="r.valuesCsv" :placeholder="t('values, comma-separated')" class="border rounded px-2 py-1 text-sm flex-1" />
                            <label class="text-xs self-center"><input type="checkbox" v-model="r.hard_exclusion" /> {{ t('hard') }}</label>
                        </div>
                    </div>
                    <div>
                    ...
                    <div v-for="c in campaigns" :key="c.campaign_id" class="p-3">
                        <div class="flex justify-between items-center">
                            ...
                            <div class="flex gap-1">
                                <button v-if="['DRAFT', 'PAUSED'].includes(c.status)" @click="campaignAction(c, 'activate')" class="px-2 py-0.5 bg-green-600 text-white rounded text-xs">{{ t('Launch') }}</button>
                                <button v-if="c.status === 'ACTIVE'" @click="campaignAction(c, 'pause')" class="px-2 py-0.5 bg-yellow-500 text-white rounded text-xs">{{ t('Pause') }}</button>
                                <button v-if="['ACTIVE', 'PAUSED'].includes(c.status)" @click="campaignAction(c, 'end')" class="px-2 py-0.5 bg-red-500 text-white rounded text-xs">{{ t('End') }}</button>
                                <button @click="eligibility.open = eligibility.open === c.campaign_id ? null : c.campaign_id; eligibility.result = null"
                                    class="px-2 py-0.5 bg-gray-200 rounded text-xs">{{ t('Test eligibility') }}</button>
                            </div>
                        </div>
            ...
            <div v-else>
                ...
                <div v-if="showBundleDesigner" class="bg-white rounded shadow p-4 mb-4 space-y-2">
                    <div class="grid grid-cols-3 gap-2">
                        ...
                        <input v-model="bundleDraft.display_name" :placeholder="t('Display name')" class="border rounded px-2 py-1 text-sm" />
                    ...
                    <input v-model="bundleDraft.componentsCsv" :placeholder="t('package refs, comma-separated (first = PRIMARY mandatory)')" class="border rounded px-2 py-1 text-sm w-full" />
                    <div class="grid grid-cols-3 gap-2">
