...
| 27 | Warehouse backoffice (added) | UI over OSR | `resources/js/Pages/Osr/Warehouse.vue` on the shared kit: stock-balance/PO/equipment/swap DataTables, summary StatCards, record-movement form, equipment-lifecycle Timeline — themed + i18n | — | `BackofficeSurfacesTest` render | ✅ |
...
| 31 | Ticketing cockpit UI (added) | UI over TCK | `resources/js/Pages/Tickets/Index.vue` on the shared kit: summary StatCards, queue DataTable, detail Drawer (lifecycle actions + comments), SLA-breach badge, activity Timeline — themed + i18n | — | `TicketApiTest`, `BackofficeSurfacesTest` render | ✅ |
...
| 35 | Catalog setup (service/package/tax/wallet/tariff) | PLM/SIP/BIL config | `Modules/Catalog` models + seeders; cache-aside consumers. **SIP-02** launch lifecycle (`PackageLaunchService` + 6 tables: launch_plan/check/availability/version_cutover/lifecycle_event/retirement_plan; EM-CFG-04-gated approval via `ApplyPackageLaunchApproval`; `/packages/available` cache-aside read model). **PLM-CFG-07** voice tariff (`VoiceTariffService` + 7 tables plan/zone/prefix/time_band/rate/allowance/binding; longest-prefix + effective-date + binding-priority rating lookup; overlap validation). **PLM-CFG-02** tax admin (`TaxConfigService` effective-dated rule versioning + ordered groups). `CatalogCacheInvalidator` (event-driven eviction via `CatalogCacheKeys`). Operator **Catalog Setup** UI (`Pages/Catalog/Setup.vue`, full i18n). | CatalogChanged (evicts); PackageActivated/AvailabilityChanged/EndOfSale/Retired; VoiceTariffPlan/Rate/Prefix/Binding; TaxConfigChanged | `PackageLaunchTest`, `VoiceTariffTest`, `TaxConfigTest`, `CacheFoundationTest` | ✅ |
...
| 42 | NOC console + end-to-end traces | ops (added) | `NocController` (overview, service control, correlation-id trace across events/instances/tasks/commands/logs/notifications); `ItOps/Noc.vue` on the shared kit: overview StatCards, service/SLA DataTables, **correlation-id trace as a Timeline** — themed + i18n | — | `NocConsoleTest` (4) | ✅ |
...
| 46 | Reporting (thin; deep = external) | REP-01 | `report_daily_metric` mart projected from outbox events (`ReportMetricProjector`); `ReportController` dashboards/metrics/export/reconcile; **Reports surface** (`Reports/Dashboard.vue`): dashboard selector + date range, metric StatCards, themed SVG `BarChart` time-series, CSV export, mart-vs-event-log reconciliation status — i18n + operator theme | consumes events | `ReportExportReconcileTest` (3); `BackofficeSurfacesTest` render | ✅ |

### 1.7 Channels (frontend DDs)

| # | Feature | Command owner (DD) | Implementation entry points | Events | E2E proof (tests) | Tour |
|---|---|---|---|---|---|---|
| 47 | Backoffice web app (12 surfaces) | FE-APP-01 | Sidebar shell (`AuthenticatedLayout`: role-aware nav gated by Inertia-shared permissions, operator theming, env marker, breadcrumb via `PageHeader`, global search, richer profile) + shared visual kit `resources/js/Components/Bss/*` (StatusBadge/StatCard/StageTracker/Timeline/Panel/DataTable/Drawer/PageHeader) + operator theme utilities (`--op-primary`, `.bg-op*`). Surfaces: Home Dashboard (6 resilient widgets + onboarding funnel + activity feed via `/api/dashboard/summary`), Subscriptions (MACD + lifecycle StageTracker), Billing (invoices/payments/adjustments/wallet/dunning/tax), Fulfillment (onboarding StageTracker showcase), Work Orders (kanban dispatch board), Workforce, Equipment (instances/swap/RMA), Catalog Setup, plus existing Customers/Tickets/Reports/Admin/NOC/IT-Ops | — | `BackofficeSurfacesTest` (dashboard summary + Inertia render of 9 surfaces) | ✅ |
...
| 2026-06-12 | #35 catalog setup (full depth) | SIP-02 launch lifecycle (6 tables, state machine, EM-CFG-04 approval, version cutover, availability suspend/resume, retirement, `/packages/available` read model); PLM-CFG-07 voice tariff (7 tables, binding-priority + longest-prefix + effective-date rating lookup, overlap validation); PLM-CFG-02 tax admin (effective-dated rule versioning); event-driven `CatalogCacheInvalidator`; operator Catalog Setup UI (i18n). Drools→RuleEngine, Kafka→outbox, Camunda→ApprovalService (suite →440) |
| 2026-06-12 | #47 backoffice (FE-APP-01, all surfaces) | sidebar shell (role-aware nav, env marker, breadcrumb, global search, richer profile) + shared visual kit (StageTracker/Timeline/DataTable/Drawer/StatCard/Panel/StatusBadge/PageHeader) + operator theme utilities (`--op-primary`/`.bg-op*`); built Home Dashboard (resilient `/api/dashboard/summary` widgets + onboarding funnel), Subscriptions, Billing, Fulfillment (onboarding visual), Work Orders (kanban), Workforce, Equipment, over existing module APIs; full i18n; `BackofficeSurfacesTest` (render proof for 9 surfaces) |
| 2026-06-12 | theming + i18n sweep | every brand accent across all 27 views + the kit now resolves to operator `theme_primary_color` via `--op-primary` utilities (zero hardcoded brand colour); confirmed i18n on all surfaces |
| 2026-06-12 | #46 reporting (REP-01) | Reports surface over the existing event-projected mart: dashboard selector + range, metric StatCards, themed SVG BarChart time-series, CSV export, reconciliation status; i18n + theme (suite →442) |
| 2026-06-12 | FE-APP-01 §16 global search | federated `/api/search` across customer/account/subscription/invoice/payment/work-order/equipment/ticket (per-source isolation, operator-scoped); live debounced dropdown in the shell with type→deep-link routing (closes the #47 search stub) |
| 2026-06-12 | #47 gap closures | payment `show` + index eager-load allocations (allocation breakdown in Billing drawer); platform-wide restriction monitor `/api/subscription-restrictions` (Fulfillment restriction tab); cycle-close run monitor `/api/cycle-close-runs` (Billing cycle tab); `BackofficeGapClosuresTest` (3) |
| 2026-06-12 | #31/#42 onto kit | Tickets cockpit (StatCards/DataTable/Drawer/SLA-badge/Timeline) + NOC console (StatCards/DataTables, correlation-id trace as a Timeline) restyled on the shared kit, themed + i18n, data layers preserved (suite 446) |
| 2026-06-12 | backoffice demo dataset | `BackofficeDemoSeeder` (wired into `sophix:setup demo`): customers across journey stages, in-flight onboarding orders, a restricted subscription, tickets, reporting metrics — so the dashboard funnel, search, monitors and reports open populated; `BackofficeDemoSeederTest` |
| 2026-06-12 | #35 catalog setup (full depth) | built SIP-02 package launch lifecycle (6 tables, EM-CFG-04 approval, version cutover, availability, end-of-sale/retirement, scheduled-activation worker, cache-aside `/packages/available`) + PLM-CFG-07 voice tariff (7 tables, longest-prefix/effective-date/binding-priority rating lookup, overlap validation, snapshot cache) + PLM-CFG-02 tax admin (effective-dated rule versioning, ordered groups) + event-driven `CatalogCacheInvalidator` + operator Catalog Setup UI (i18n). Drools→RuleEngine, Kafka→outbox, Camunda→ApprovalService throughout |
