...
echo "=== workflow palette (raw top-level keys + descriptions):"
curl -s "${H[@]}" "https://studio-workflow.$BASE/api/workflow/palette" | python3 -c '
import sys,json
d=json.load(sys.stdin)
d=d.get("data", d)   # tolerate either wrapped or top-level
...
if nt: print("  e.g.", nt[0]["type"], "->", nt[0].get("description","")[:80])
if st: print("  e.g.", st[0]["topic"], "->", st[0].get("description","")[:80])
'

echo "=== studio pages — actual Inertia component (from data-page):"
cat >/tmp/comp.py <<'PY'
...
for slug in studio-workflow:workflow/studio studio-rules:rules/studio studio-dunning:dunning/studio; do
  sub=${slug%%:*}; path=${slug##*:}
  echo -n "  $sub/$path"; curl -sL -b "$JAR" "https://$sub.$BASE/$path" | python3 /tmp/comp.py
done
