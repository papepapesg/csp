...
async function load() {
    ...
    try {
        ...
        error.value = e.response?.data?.message ?? 'Failed to load dashboards';
    } finally {
        loading.value = false;
    }
}
...
<template>
    ...
    <AuthenticatedLayout>
        <template #header>
            <h2 class="text-xl font-semibold leading-tight text-gray-800">Reporting (REP-01)</h2>
        </template>
        ...
        <div class="py-8">
            <div class="mx-auto max-w-7xl space-y-8 sm:px-6 lg:px-8">
                ...
                <template v-else>
                    <section>
                        <h3 class="mb-3 text-sm font-semibold uppercase text-gray-500">Operations</h3>
                        <div class="grid grid-cols-2 gap-4 sm:grid-cols-4">
                            <div v-for="(v, k) in ops" :key="k" class="rounded-lg bg-white p-5 shadow-sm">
                                ...
                                <div class="mt-1 text-xs text-gray-500">{{ labels[k] ?? k }}</div>
                            </div>
                        </div>
                    </section>

                    <section>
                        <h3 class="mb-3 text-sm font-semibold uppercase text-gray-500">Revenue</h3>
                        <div class="grid grid-cols-2 gap-4 sm:grid-cols-4">
                    ...
                    <p class="text-xs text-gray-400">
                        Metrics are projected from committed domain events via the transactional outbox.
                        Run the queue + <code>sophix:outbox:dispatch</code> to refresh.
                    </p>
                </template>
            </div>
        </div>
